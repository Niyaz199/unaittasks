...
import {
  ...
  listStockLocationsObjectSummariesForProfile,
...
import { StockLocationsObjectHub } from "@/components/warehouse/stock-locations-object-hub";
...
export default async function WarehouseLocationsPage({
  ...
  const [locations, systems, rooms, summaries] = await Promise.all([
    shouldShowCatalog ? listStockLocationsForProfile(supabase, profile, { objectId: selectedObjectId }) : Promise.resolve([]),
    ...
    shouldShowCatalog ? Promise.resolve([]) : listStockLocationsObjectSummariesForProfile(supabase, profile),
  ]);

  const objectName = objects.find((o) => o.id === selectedObjectId)?.name ?? "";
  const description = shouldShowCatalog
    ? `«${objectName}»: шкафы, кладовые, стеллажи и другие места хранения с QR-кодами.`
    : "Выберите объект, чтобы открыть его места хранения.";
  ...
  return (
    <section className="grid">
      ...
      {shouldShowCatalog ? (
        ...
        <StockLocationsObjectHub summaries={summaries} />
      )}
    </section>
  );
}
