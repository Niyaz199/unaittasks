...
export function PprAssignmentsAdmin({
  ...
  const filteredAssignments = useMemo(() => {
    return assignments.filter((assignment) => {
      const equipLabel = resolveEquipmentLabel(assignment.equipment).toLowerCase();
      const templateLabel = resolveTemplateLabel(assignment.template).toLowerCase();
      const searchLower = searchTerm.toLowerCase();
      
      const matchesSearch =
        searchTerm === "" ||
        equipLabel.includes(searchLower) ||
        templateLabel.includes(searchLower);
        
      const matchesObject = filterObjectId === "" || assignment.object_id === filterObjectId;
      const matchesStatus =
        filterStatus === "all" ||
  ...
  }, [assignments, searchTerm, filterObjectId, filterStatus]);

  const metrics = useMemo(() => {
    return [
      { label: "Всего назначений", value: assignments.length, tone: "neutral" as const },
      { label: "Активных", value: assignments.filter((a) => a.is_active).length, tone: "success" as const },
    ];
  }, [assignments]);
  ...
  return (
    <>
      <PprPageShell
        ...
        searchPlaceholder="Поиск по оборудованию или шаблону..."
        isEmpty={!hasPrerequisites || assignments.length === 0}
        emptyState={{
          message: !hasPrerequisites ? "Недостаточно данных для создания назначения" : "Назначения ППР пока не созданы",
          hint: !hasPrerequisites ? "Для назначения нужны доступное оборудование и шаблоны ППР из предыдущих батчей." : "Свяжите шаблон с оборудованием и задайте стартовую дату.",
        }}
        isFilteredEmpty={filteredAssignments.length === 0}
        filters={
          <>
            <select
              className="select"
