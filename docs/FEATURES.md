...
- глобальный справочник этажей;
- глобальный справочник типов помещений;
- справочник групп систем;
- справочник систем;
- справочник помещений с привязкой к этажу и типу помещения;
...
- удаление объекта;
- список этажей;
- создание, изменение и удаление этажей с защитой от удаления используемых записей;
- список типов помещений;
- создание, изменение и удаление типов помещений с защитой от удаления используемых записей;
...
- подтверждение помещения по QR через `/rounds/scan?token=...`;
- `/rounds/today` со статусом "отмечено / не отмечено" по помещениям;
- `/rounds/archive` с фильтрами по объекту, периоду, технику и помещению;
- `/rounds/config` для массового включения помещений в обходы;
- `/rounds/qr` для просмотра, печати и поштучной выгрузки QR;
- QR для помещений на базе общего справочника `object_rooms`;
- upsert check-in по `room_id + operational_date` с правилом newer-wins по `scanned_at_device`;
- отдельный scanner read-model для техников через RPC;
- офлайн-очередь обходов с синком фото и ручной кнопкой синхронизации.
...
- bucket `rounds-files`;
- signed URLs для выдачи в today/archive.
...
- отдельная очередь обходов с фото;
- синк обходов при `online`, `focus`, `visibilitychange` и вручную.
...
- команда задачи;
- справочники;
- админские операции задач и ППР.
...
- fallback-кэш для `/rounds` и `/rounds/scan`.

Не реализовано как полноценная возможность:

- полноценный offline-browse всех экранов;
- offline-first data cache для задач или ППР вне обходов;
- универсальная очередь для всех mutating-операций вне `tasks` и `rounds`.
...
- модель помещений с глобальными этажами и типами помещений.
