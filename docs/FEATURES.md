...
- страница `/ppr/rooms`;
- импорт новых помещений из `CSV` и `XLSX` с предпросмотром, построчной валидацией и confirm-step;
...
- импорт помещений не обновляет существующие записи и не создаёт новые объекты, этажи или типы автоматически;
- `CSV` и `XLSX` используют один и тот же preview/commit pipeline после parse-слоя.
...
- чек-листы шаблонов;
- системные шаблоны ППР для всего оборудования выбранной системы;
- календарь ППР;
...
- `done` требует минимум один комментарий (фото — опционально);
