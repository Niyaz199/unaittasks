...
- **Создание задачи** — объект, приоритет, срок, исполнитель, команда; push назначаемому; фото-вложения
...
- **Фото-вложения** — к задаче и к комментариям; Supabase Storage; превью + лайтбокс
...
Поддержка фото-вложений добавлена в форму создания задачи и в форму комментариев.

### Где работают

- **Создание задачи** (`/tasks/create`) — секция «Фото-вложения» в конце формы. Фото загружаются после успешного создания задачи. Прикреплять фото необязательно.
- **Комментарий к задаче** — кнопка «Прикрепить фото к комментарию» под textarea. Фото загружаются после отправки комментария.
- **Карточка задачи** — в разделе «Описание» отображаются вложения задачи; под каждым комментарием — его вложения.

### Лимиты

| Параметр | Значение |
|----------|----------|
| Макс. файлов за раз | 5 |
| Макс. размер одного файла | 5 MB |
| Разрешённые типы | `image/jpeg`, `image/png`, `image/webp` |
| PDF, видео, прочее | Запрещено |

Нарушение лимитов показывает понятное сообщение об ошибке до отправки (клиентская валидация) и при попытке загрузки через API (серверная валидация).

### Хранение

Файлы хранятся в Supabase Storage в bucket **`task-attachments`** (приватный, без публичного доступа).  
В базе данных хранится только таблица метаданных `task_attachments` — пути, имена, MIME-типы, размеры и связи с задачами/комментариями. Бинарные данные в БД не хранятся.

Для отображения используются **signed URLs** (время жизни 1 час), генерируемые сервером по запросу.

### Доступ

Политики RLS bucket и таблицы метаданных повторяют текущую модель доступа к задачам: файл видит тот, кто имеет доступ к соответствующей задаче.

### Каркас под автоудаление (не активно)

В таблице `task_attachments` есть поле `cleanup_after` — дата, после которой файл может быть удалён. Заполняется автоматически при загрузке: `created_at + 30 дней`. Также есть поле `deleted_at` для мягкого удаления.

**Фактическое удаление сейчас НЕ выполняется.** Поле заполнено только как основа для будущей задачи.

Чтобы включить автоудаление в будущем, достаточно создать scheduled job/cron:

```sql
-- Пример: найти просроченные файлы
select storage_path from task_attachments
where cleanup_after < now()
  and deleted_at is null;
```

Варианты реализации:
- **Supabase Edge Function** по расписанию (pg_cron или внешний cron)
- **Next.js API route** `/api/cron/cleanup-attachments` с `CRON_SECRET` (по аналогии с `/api/cron/archive`)
- Любой внешний scheduler (GitHub Actions, Railway cron и т.д.)

Алгоритм: выбрать записи `cleanup_after < now()`, удалить файлы из bucket, проставить `deleted_at` в таблице.
...
# Или вручную — запустить содержимое файла в Supabase SQL Editor:
# supabase/migrations/0009_task_attachments.sql
```

После миграции создать bucket вручную в Supabase Dashboard → Storage → New bucket:
- Name: `task-attachments`
- Public bucket: **выключить**
